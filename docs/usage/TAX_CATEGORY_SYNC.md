...
final CompletableFuture<TaxCategorySyncStatistics> syncFuture1 = taxCategorySync.sync(batch1).toCompletableFuture();
final CompletableFuture<TaxCategorySyncStatistics> syncFuture2 = taxCategorySync.sync(batch2).toCompletableFuture();
...
````java
final TaxCategorySync taxCategorySync = new TaxCategorySync(syncOptions);
taxCategorySync.sync(batch1)
           .thenCompose(result -> taxCategorySync.sync(batch2))
           .toCompletableFuture()
           .join();
````
