...
public final class ShoppingListReferenceResolver
    extends CustomReferenceResolver<
        ShoppingListDraft, ShoppingListDraftBuilder, ShoppingListSyncOptions> {

  static final String FAILED_TO_RESOLVE_CUSTOMER_REFERENCE =
  ...
  /**
   * Takes a {@link ShoppingListSyncOptions} instance, a {@link CustomerService} and {@link
   * TypeService} to instantiate a {@link ShoppingListReferenceResolver} instance that could be used
   * to resolve the shopping list drafts in the CTP project specified in the injected {@link
   ...
   */
  public ShoppingListReferenceResolver(
    ...
    this.lineItemReferenceResolver =
        new LineItemReferenceResolver(shoppingListSyncOptions, typeService);
    this.textLineItemReferenceResolver =
        new TextLineItemReferenceResolver(shoppingListSyncOptions, typeService);
  ...
  private CompletionStage<ShoppingListDraftBuilder> fetchAndResolveCustomerReference(
      @Nonnull final ShoppingListDraftBuilder draftBuilder, @Nonnull final String customerKey) {

    return customerService
        .fetchCachedCustomerId(customerKey)
        .thenCompose(
            resolvedCustomerIdOptional ->
                resolvedCustomerIdOptional
                    .map(
                        resolvedCustomerId ->
                            completedFuture(
