...
public final class CartDiscountReferenceResolutionUtils {
   ...
   * </table>
   *
   * <p><b>Note:</b> The {@link Type} reference should has an expanded reference with a key. Any
   * reference that is not expanded will have its id in place and not replaced by the key will be
  ...
  /**
   * Builds a {@link CartDiscountQuery} for fetching cart discounts from a source CTP project with
   ...
   * source project.
   *
   * @return the query for fetching cart discounts from the source CTP project with all the
