...
public final class CustomTypeReferenceResolutionUtils {
    ...
    /**
     * Given a custom {@link CustomFields}, this method provides checking to certain resources which do not extends
     * {@link Custom}, such as {@link ShoppingList}, {@link LineItem} and {@link TextLineItem}. If the custom fields
     * are existing (not null) and they are reference expanded. If they are then it returns a {@link CustomFieldsDraft}
     * instance with the custom type key in place of the key of the reference. Otherwise, if it's not reference expanded
     * it returns a {@link CustomFieldsDraft} without the key. If the resource has null {@link Custom}, then it returns
     * {@code null}.
     *
