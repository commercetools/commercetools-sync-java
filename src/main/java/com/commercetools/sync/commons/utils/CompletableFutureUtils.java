...
public final class CompletableFutureUtils {
     ...
     * @param mapper    function to map each value to a {@link CompletableFuture}
     ...
     */
    @Nonnull
    public static <T, S, U extends Collection<S>> CompletableFuture<U> mapValuesToFutureOfCompletedValues(
        @Nonnull final Collection<T> values,
        @Nonnull final Function<T, CompletableFuture<S>> mapper,
    ...
    /**
     * Creates a Future containing a stream of value results after the mapper function is applied to each value in the
     * supplied collection and completed it. The type of the collection is decided upon on by the supplied collector.
     *
     * @param values collection of values to apply a mapper function that would map each to a {@link CompletableFuture}.
     * @param mapper function to map each value to a {@link CompletableFuture}
     * @param <T>    The type of the values.
     * @param <S>    The type of the mapped completed values.
     * @return a future containing a stream of completed stage results of the values after the mapper function was
     *         applied to each value in the supplied collection.
     */
    @Nonnull
    public static <T, S> CompletableFuture<Stream<S>> mapValuesToFutureOfCompletedValues(
        ...
        final List<CompletableFuture<S>> futureList = mapValuesToFutures(values.stream(), mapper, toList());
     ...
     *
     * @param values    stream of values to apply a mapper function that would map each to a {@link CompletableFuture}.
     * @param mapper    function to map each value to a {@link CompletableFuture}
     * @param collector the collector to define the type of the collection returned.
     * @param <T>       The type of the values.
    ...
    /**
     * Transforms a collection of {@code CompletableFuture} into a {@code CompletableFuture} of a collection,
     ...
     *
     * @param futures   collection of {@code CompletableFuture}
     * @param collector the collector to define the type of the collection returned.
     * @param <T>       the element obtained from the set of {@code CompletableFuture}
     * @param <S>       The type of the collection returned in the future.
     * @return the {@code CompletableFuture} of a collection of elements
     */
    @Nonnull
    public static <T, S extends Collection<T>> CompletableFuture<S> collectionOfFuturesToFutureOfCollection(
        @Nonnull final Collection<? extends CompletableFuture<T>> futures,
        @Nonnull final Collector<T, ?, S> collector) {

        final List<CompletableFuture<T>> futureList = futures.stream()
    ...
    public static <T, S, U extends Collection<CompletableFuture<S>>> U mapValuesToFutures(
        ...
        final Stream<CompletableFuture<S>> stageStream = filterNullAndMap(values, mapper);
        return toCompletableFutures(stageStream, collector);
    }

    /**
     * Converts a stream of {@link CompletableFuture} of values of type {@code <T>} into a
     * {@link Collection} of the type of the supplied {@code collector} of {@link CompletableFuture}
     * of values of type {@code <T>}.  The type of the returned collection is decided by the supplied collector.
     *
     * <p>Note: Null futures in the stream are filtered out.
     *
     * @param values    stream of {@link CompletableFuture} of values of type {@code <T>}
     * @param collector the collector to define the type of the collection returned.
    ...
    public static <T, S extends Collection<CompletableFuture<T>>> S toCompletableFutures(
        @Nonnull final Stream<CompletableFuture<T>> values,
        @Nonnull final Collector<CompletableFuture<T>, ?, S> collector) {
        return values.filter(Objects::nonNull)
                     .map(CompletableFuture::toCompletableFuture).collect(collector);
    }
