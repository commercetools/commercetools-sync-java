...
class GenericUpdateActionUtils {
    ...
    /**
     * Creates a SyncResult containing a "setCustomType" update action on the given resource {@link T} (which currently
     * could either be a {@link Category} or a {@link Channel}).
     *
     ...
     * @return A sync result with a list containing the setCustomType update action of the type of the resource it's
     * requested on.
     */
    @Nonnull
    static <T extends Custom> SyncResult<T> buildTypedSetCustomTypeUpdateAction(
    ...
    /**
     * Creates a SyncResult containing a "setCustomType" update action on the given resource {@link T} that removes the custom type set on
     * the given resource {@link T} (which currently could either be a {@link Category} or a {@link Channel}).
     *
     * @param resource the resource to do the update action on.
     * @param <T>      the type of the resource to do the update action on.
     * @return A sync result with a list containing the setCustomType update action that
     * removes the custom type from the resource it's requested on.
     */
    @Nonnull
    static <T extends Custom> SyncResult<T> buildTypedRemoveCustomTypeUpdateAction(
            @Nonnull final T resource) {
        return buildTypedUpdateAction(resource, SET_CUSTOM_TYPE_REMOVE);
    }

    /**
     * Creates a SyncResult containing a "setCustomField" update action on the given resource {@link T} that
     * updates a custom field with {@code customFieldName} and a {@code customFieldValue} on the given
     ...
     * @param resource         the resource to do the update action on.
     * @param <T>              the type of the resource to do the update action on.
     * @return A sync result with a list containing the setCustomField update action with the provided value
     * on the resource it's requested on.
     */
    @Nonnull
    static <T extends Custom> SyncResult<T> buildTypedSetCustomFieldUpdateAction(
    ...
    @Nonnull
    private static <T extends Custom> SyncResult<T> buildTypedUpdateAction(
            @Nullable final String customTypeKey,
            @Nullable final Map<String, JsonNode> customFieldsJsonMap,
            @Nonnull final T resource,
    ...
    @Nonnull
    static <T extends Custom> SyncResult<T> buildTypedUpdateAction(
            @Nonnull final T resource,
            @Nonnull final String updateActionName) {
    ...
    /**
     * Creates a SyncResult containing a CTP update action on the given
     * resource {@link T} (which currently could either be a {@link Category} or a {@link Channel}) according to
     * the {@code updateActionName} flag. According to this flag value, the required update action is built:
     * <ol>
     ...
     * @param <T>                 the type of the resource to do the update action on.
     * @return A sync result with a list containing the update action that depends on the provided flag on the resource
     * it's requested on.
     */
    @Nonnull
    ...
    private static <T extends Custom> SyncResult<T> buildTypedUpdateAction(
        ...
        if (resource instanceof Category) {
            switch (updateActionName) {
                case SET_CUSTOM_TYPE_REMOVE:
                    return SyncResult.ofUpdateAction((UpdateAction<T>)
                ...
                case SET_CUSTOM_FIELD:
                    return SyncResult.ofUpdateAction((UpdateAction<T>) io.sphere.sdk.categories.commands.updateactions
        ...
        if (resource instanceof Channel) {
            switch (updateActionName) {
                case SET_CUSTOM_TYPE_REMOVE:
                    return SyncResult.ofUpdateAction((UpdateAction<T>) io.sphere.sdk.channels.commands.updateactions
