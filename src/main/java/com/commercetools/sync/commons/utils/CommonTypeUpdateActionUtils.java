...
public final class CommonTypeUpdateActionUtils {
  ...
  /**
   * Compares two objects that are of type {@link ResourceIdentifier} (or a type that extends it)
   * and returns a supplied {@link UpdateAction} as a result in an {@link Optional}. If both the
   * {@link Object}s have the same values, then no update action is needed and hence an empty {@link
  ...
  @Nonnull
  public static <
          T, S extends ResourceIdentifier, U extends ResourceIdentifier, V extends UpdateAction<T>>
  ...
  /**
   * Compares the ids of two objects that are of type {@link ResourceIdentifier} (or a type that
   * extends it).
   *
   * @param oldResourceIdentifier the old resource identifier
   * @param newResourceIdentifier the new resource identifier
   ...
   */
  public static <T extends ResourceIdentifier, S extends ResourceIdentifier>
      boolean areResourceIdentifiersEqual(
          @Nullable final T oldResourceIdentifier, @Nullable final S newResourceIdentifier) {

    final String oldId =
        ofNullable(oldResourceIdentifier).map(ResourceIdentifier::getId).orElse(null);
    final String newId =
  ...
  /**
   * Compares two {@link Object} and returns a supplied list of {@link UpdateAction} as a result. If
   * both the {@link Object}s have the same values, then no update action is needed and hence an
