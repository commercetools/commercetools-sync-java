...
public final class CustomUpdateActionUtils {
  ...
  public static <T extends Custom & Resource<T>, S extends CustomDraft>
      List<UpdateAction<T>> buildPrimaryResourceCustomUpdateActions(
        ...
        if (newResourceCustomFields != null) {
          ...
          if (isBlank(newCustomFieldsTypeId)) {
            ...
            final Map<String, JsonNode> newCustomFieldsJsonMap =
                newResourceCustomFields.getFields();
            final Optional<UpdateAction<U>> updateAction =
  ...
  @Nonnull
  private static <D, T extends Custom, S extends CustomDraft, U extends Resource<U>>
      List<UpdateAction<U>> buildCustomUpdateActions(
          @Nonnull final T oldResource,
          @Nonnull final S newResourceDraft,
          @Nonnull final GenericCustomActionBuilder<U> customActionBuilder,
   ...
   * {@link List}&lt;{@link UpdateAction}&gt; as a result. The keys are used to compare the custom
   * types. The key of the old resource custom type is fetched from the caching mechanism of the
   * {@link TypeService} instance supplied as a param to the method. The key of the new resource
   * custom type is expected to be set on the type. If no update action is needed an empty {@link
   * List}&lt;{@link UpdateAction}&gt; is returned.
   *
   * <p>An update action will be added to the result list in the following cases:-
   *
   * <ol>
   ...
   * List}&lt;{@link UpdateAction}&gt; as a result. If no update action is needed an empty {@link
   * List}&lt;{@link UpdateAction}&gt; is returned.
   *
   * <p>An update action will be added to the result list in the following cases:-
   *
   ...
   * UpdateAction}&gt; as a result. If no update action is needed an empty {@link List}&lt;{@link
   * UpdateAction}&gt; is returned.
   *
   ...
   * resource's custom fields. It returns a {@link List}&lt;{@link UpdateAction}&gt; as a result. If
   * no update action is needed an empty {@link List}&lt;{@link UpdateAction}&gt; is returned.
   *
   * @param <T> the type of the old {@link Resource} which has the custom fields.
   * @param <U> the type of the resource in which the update actions will be applied on.
   * @param oldCustomFields the old resource's custom fields map of JSON values.
