...
final class GraphQlQueryAll {
  ...
  private static ResourceKeyIdGraphQlRequest withDefaults(
      @Nonnull final ResourceKeyIdGraphQlRequest resourceKeyIdGraphQlRequest, final long pageSize) {

    return resourceKeyIdGraphQlRequest.withLimit(pageSize);
  }

  @Nonnull
  static GraphQlQueryAll of(
      @Nonnull final SphereClient client,
      @Nonnull final ResourceKeyIdGraphQlRequest resourceKeyIdGraphQlRequest,
      final int pageSize) {
  ...
  /**
   * Given a {@link Consumer} to a page of results of type {@code BaseGraphQlResult}, this method
   * sets this instance's {@code pageConsumer} to the supplied value, then it makes requests to
   * fetch the entire result space of a graphql query request {@code BaseGraphQlRequest} to CTP,
   * while accepting the consumer on each fetched page.
   *
  ...
  CompletionStage<Void> run(@Nonnull final Consumer<Set<ResourceKeyId>> pageConsumer) {
    ...
    final CompletionStage<ResourceKeyIdGraphQlResult> firstPage =
        client.execute(resourceKeyIdGraphQlRequest);
    return queryNextPages(firstPage);
  }

  /**
   * Given a completion stage {@code currentPageStage} containing a current graphql result {@link
   * ResourceKeyIdGraphQlResult}, this method composes the completion stage by first checking if the
   * result is null or not. If it is not, then it recursively (by calling itself with the next
   * page's completion stage result) composes to the supplied stage, stages of the all next pages'
   * processing. If there is no next page, then the result of the {@code currentPageStage} would be
   * null and this method would just return a completed future containing null result, which in turn
   * signals the last page of processing.
   *
   * @param currentPageStage a future containing a graphql result {@link
  ...
  private CompletionStage<Void> queryNextPages(
    ...
    return currentPageStage.thenCompose(
        currentPage ->
            currentPage != null
                ? queryNextPages(processPageAndGetNext(currentPage))
                : completedFuture(null));
  }

  /**
   * Given a graphql query result representing a page {@link ResourceKeyIdGraphQlResult}, this
   * method checks if there are elements in the result (size > 0), then it consumes the resultant
   * list using this instance's {@code pageConsumer}. Then it attempts to fetch the next page if it
   * exists and returns a completion stage containing the result of the next page. If there is a
   * next page, then a new future of the next page is returned. If there are no more results, the
  ...
  /**
   * Given a list of page elements of type {@code ResourceKeyId}, this method checks if this page is
   * the last page or not by checking if the result size is equal to this instance's {@code
   * pageSize}). If it is, then it means there might be still more results. However, if not, then it
   * means for sure there are no more results and this is the last page. If there is a next page,
   * the id of the last element in the list is used and a future is created containing the fetched
   * results which have an id greater than the id of the last element in the list and this future is
  ...
  private CompletionStage<ResourceKeyIdGraphQlResult> getNextPageStage(
    ...
    if (pageElements.size() == pageSize) {
      ...
      final String queryPredicate =
          isBlank(lastElementId) ? null : format("id > \\\\\\\"%s\\\\\\\"", lastElementId);
