...
final class QueryAll<T extends ResourceView, C extends QueryDsl<T, C>, S> {
  ...
  /**
   * Given a {@link Function} to a page of resources of type {@code T} that returns a mapped result
   * of type {@code S}, this method sets this instance's {@code pageMapper} to the supplied value,
   * then it makes requests to fetch the entire result space of the resource {@code T} on CTP, while
  ...
  /**
   * Given a {@link Consumer} to a page of resources of type {@code T}, this method sets this
   * instance's {@code pageConsumer} to the supplied value, then it makes requests to fetch the
   * entire result space of the resource {@code T} on CTP, while accepting the consumer on each
   * fetched page.
   *
   ...
   * PagedQueryResult}, this method composes the completion stage by first checking if the result is
  ...
  private CompletionStage<Void> queryNextPages(
      @Nonnull final CompletionStage<PagedQueryResult<T>> currentPageStage) {
  ...
  /**
   * Given a page result {@link PagedQueryResult}, this method checks if there are elements in the
   ...
   */
  @Nonnull
  private CompletionStage<PagedQueryResult<T>> processPageAndGetNext(
      @Nonnull final PagedQueryResult<T> page) {
  ...
  /**
   * Given a list of page elements of resource {@code T}, this method checks if this instance's
  ...
  /**
   * Given a list of page elements of resource {@code T}, this method checks if this page is the
   ...
   */
  @Nonnull
  private CompletionStage<PagedQueryResult<T>> getNextPageStage(
      @Nonnull final List<T> pageElements) {
    if (pageElements.size() == pageSize) {
      ...
      final QueryPredicate<T> queryPredicate =
          QueryPredicate.of(format("id > \"%s\"", lastElementId));
