...
public interface ProductTypeReferenceTransformService {
   ...
   * for the reference id's) non null and unexpanded references of the productType{@link ProductType} by
   * using cache.
   *
   ...
   *
   * <p>Then maps the ProductType to ProductTypeDraft by performing reference resolution considering idToKey
   * value from the cache.
   *
   * @param productTypes the productTypes to replace the references and attributes id's with keys.
   * @return a new list which contains productTypeDrafts which have all their references and attributes
   *     references resolved and already replaced with keys.
   */
  @Nonnull
  CompletableFuture<List<ProductTypeDraft>> transformProductTypeReferences(@Nonnull List<ProductType> productTypes);
}
