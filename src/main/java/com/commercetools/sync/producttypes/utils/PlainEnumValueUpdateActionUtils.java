...
public final class PlainEnumValueUpdateActionUtils {
   ...
   * changeEnumValueOrder) and 1-1 update actions on enum values (e.g. changeEnumValueLabel) for the
   * required resource. If both the {@link EnumValue}'s are identical, then no update action is
  ...
  /**
   * Compares all the fields of an old {@link EnumValue} and a new {@link EnumValue} and returns a
   * list of {@link UpdateAction}&lt;{@link ProductType}&gt; as a result. If both {@link EnumValue}
  ...
  /**
   * Compares the {@code label} values of an old {@link EnumValue} and a new {@link EnumValue} and
   * returns an {@link Optional} of update action, which would contain the {@code "changeLabel"}
   * {@link UpdateAction}. If both, old and new {@link EnumValue} have the same {@code label}
