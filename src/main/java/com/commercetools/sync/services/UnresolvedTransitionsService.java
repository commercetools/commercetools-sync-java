...
public interface UnresolvedTransitionsService {
    ...
    /**
     * Given state draft keys, this method fetches the persisted drafts waiting to be resolved. If there is no
     ...
     * @return {@link CompletableFuture}&lt;{@link Set}&gt; in which the result of its completion contains a
     *         {@link Set} that contains the matching drafts if any exist, otherwise empty.
     */
    @Nonnull
    CompletableFuture<Set<WaitingToBeResolvedTransitions>> fetch(@Nonnull final Set<String> keys);
     ...
     */
    @Nonnull
    CompletableFuture<Optional<WaitingToBeResolvedTransitions>> save(@Nonnull final WaitingToBeResolvedTransitions draft);
     ...
     * @return a {@link CompletableFuture} containing an optional with the deleted resource if successful otherwise an
     *     empty optional.
     */
    @Nonnull
    CompletableFuture<Optional<WaitingToBeResolvedTransitions>> delete(@Nonnull final String key);
}
