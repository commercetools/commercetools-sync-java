package com.commercetools.sync.sdk2.products.service.impl;
...
public class ProductTransformServiceImpl extends BaseTransformServiceImpl
  ...
  private CompletableFuture<Void> transformCategoryReference(
    ...
    final Set<String> categoryIds =
        products.stream()
            ...
            .map(
                categories ->
                    categories.stream().map(CategoryReference::getId).collect(Collectors.toList()))
  ...
  /**
   * Given a {@link Set}s of references of product attributes, this method first checks if there is
   * a key mapping for each id in the {@code idToKey} cache. If there exists a mapping for all the
   * ids, the method returns a future containing the existing {@code idToKey} cache as it is. If
   * there is at least one missing mapping, it attempts to make a GraphQL request (note: rest
   * request for custom objects) to CTP to fetch all ids and keys of every missing product,
   * category, productType or custom object Id in a combined request. For each fetched key/id pair,
   * the method will insert it into the {@code idToKey} cache and then return the cache in a {@link
   * CompletableFuture} after the request is successful.
   *
   * @param allAttributeReferences all references of product attributes to find a id -> key mapping
   *     for.
