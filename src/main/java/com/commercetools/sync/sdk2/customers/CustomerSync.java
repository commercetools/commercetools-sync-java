...
import com.commercetools.sync.sdk2.customers.helpers.CustomerBatchValidator;
import com.commercetools.sync.sdk2.customers.helpers.CustomerReferenceResolver;
...
public class CustomerSync
  ...
  private static final String FAILED_TO_PROCESS =
      "Failed to process the CustomerDraft with key:'%s'. Reason: %s";
  private static final String CTP_CUSTOMER_UPDATE_FAILED =
      "Failed to update customer with key: '%s'. Reason: %s";
  ...
  /**
   * Takes a {@link CustomerSyncOptions} to instantiate a new {@link CustomerSync} instance that
   * could be used to sync customer drafts in the CTP project specified in the injected {@link
   * CustomerSync} instance.
   *
   * @param customerSyncOptions the container of all the options of the sync process including the
  ...
  /**
   * Takes a {@link CustomerSyncOptions} and service instances to instantiate a new {@link
   * CustomerSync} instance that could be used to sync customer drafts in the CTP project specified
   * in the injected {@link CustomerSyncOptions} instance.
   *
   ...
   *     the CTP project.
   * @param customerGroupService the customer group service which is responsible for
   *     fetching/caching the CustomerGroups from the CTP project.
   */
  protected CustomerSync(
      @Nonnull final CustomerSyncOptions customerSyncOptions,
    ...
    this.referenceResolver =
        new CustomerReferenceResolver(getSyncOptions(), typeService, customerGroupService);
    this.batchValidator = new CustomerBatchValidator(getSyncOptions(), getStatistics());
  }

  /**
   * Iterates through the whole {@code customerDrafts} list and accumulates its valid drafts to
   * batches. Every batch is then processed by {@link CustomerSync#processBatch(List)}.
   *
   * <p><strong>Inherited doc:</strong> {@inheritDoc}
   *
   * @param customerDrafts {@link List} of {@link CustomerDraft}'s that would be synced into CTP
   *     project.
   * @return {@link CompletionStage} with {@link CustomerSyncStatistics} holding statistics of all
   *     sync processes performed by this sync instance.
   */
  @Override
  protected CompletionStage<CustomerSyncStatistics> process(
      @Nonnull final List<CustomerDraft> customerDrafts) {
    final List<List<CustomerDraft>> batches =
        batchElements(customerDrafts, syncOptions.getBatchSize());
    return syncBatches(batches, completedFuture(statistics));
  }

  @Override
  protected CompletionStage<CustomerSyncStatistics> processBatch(
      @Nonnull final List<CustomerDraft> batch) {
  ...
  private CompletionStage<Void> syncBatch(
    ...
    final Map<String, Customer> oldCustomerMap =
        oldCustomers.stream().collect(toMap(Customer::getKey, identity()));

    return CompletableFuture.allOf(
        newCustomerDrafts.stream()
            .map(
                customerDraft ->
                    referenceResolver
                        .resolveReferences(customerDraft)
                        .thenCompose(
                            resolvedCustomerDraft ->
                                syncDraft(oldCustomerMap, resolvedCustomerDraft))
                        .exceptionally(
                            completionException -> {
                              final String errorMessage =
                                  format(
  ...
  private CompletionStage<Void> buildActionsAndUpdate(
      @Nonnull final Customer oldCustomer, @Nonnull final CustomerDraft newCustomerDraft) {

    final List<CustomerUpdateAction> updateActions =
        buildActions(oldCustomer, newCustomerDraft, syncOptions);

    final List<CustomerUpdateAction> updateActionsAfterCallback =
  ...
  private CompletionStage<Void> updateCustomer(
      ...
      @Nonnull final CustomerDraft newCustomerDraft,
      @Nonnull final List<CustomerUpdateAction> updateActionsAfterCallback) {
