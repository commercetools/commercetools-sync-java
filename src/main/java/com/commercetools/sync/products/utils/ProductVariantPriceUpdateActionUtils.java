...
public final class ProductVariantPriceUpdateActionUtils {
    ...
    private static final String VARIANT_CHANGE_PRICE_EMPTY_VALUE =
        "Cannot unset 'value' field of price with id"
            + " '%s'.";
     ...
     * @param syncOptions responsible for supplying the sync options to the sync utility method. It
     *      is used for triggering the error callback within the utility, in case of errors.
     ...
     * <ul>
     * <li>{@link Price#getValue()} and {@link PriceDraft#getValue()}</li>
     ...
     * be returned in an
     * {@link Optional}, otherwise if both are identical in the {@link Price} and the {@link
     * PriceDraft}, then no update action is needed and hence an empty {@link Optional} is
     * returned.
     *
     * @param oldPrice the price which should be updated.
    ...
    public static Optional<ChangePrice> buildChangePriceUpdateAction(
        ...
        final Optional<ChangePrice> actionAfterValuesDiff = buildUpdateAction(oldPriceValue,
            newPriceValue,
     ...
     * Price} and the {@link PriceDraft} have identical custom fields and types, then no update
