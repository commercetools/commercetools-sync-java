...
public class TypeBatchValidator
  ...
  static final String TYPE_DRAFT_KEY_NOT_SET =
   ...
   * drafts and return an {@link ImmutablePair}&lt;{@link Set}&lt;{@link TypeDraft}&gt;,{@link
   * Set}&lt;{@link String} &gt;&gt; which contains the {@link Set} of valid drafts and valid type
   * keys.
   *
   ...
   * @return {@link ImmutablePair}&lt;{@link Set}&lt;{@link TypeDraft}&gt;, {@link Set}&lt;{@link
   *     String}&gt;&gt; which contains the {@link Set} of valid drafts and valid type keys.
   */
  @Override
  public ImmutablePair<Set<TypeDraft>, Set<String>> validateAndCollectReferencedKeys(
      @Nonnull final List<TypeDraft> typeDrafts) {

    final Set<TypeDraft> validDrafts =
        typeDrafts.stream().filter(this::isValidTypeDraft).collect(Collectors.toSet());

    final Set<String> validKeys = validDrafts.stream().map(TypeDraft::getKey).collect(toSet());

    return ImmutablePair.of(validDrafts, validKeys);
  }

  private boolean isValidTypeDraft(@Nullable final TypeDraft typeDraft) {
