...
class StateReferenceResolverTest {
    ...
    void resolveReferences_WithStateKeys_ShouldResolveReferences() {
        ...
        final List<State> states  = IntStream.range(0, nStates)
            .mapToObj(i -> i + "")
            .map(key -> {
        ...
        final Set<Reference<State>> stateReferences = states.stream()
                                                             .map(State::toReference)
                                                             .collect(Collectors.toSet());

        final StateDraft stateDraft = StateDraftBuilder.of("state-key", StateType.LINE_ITEM_STATE)
        ...
        // test
        final StateDraft resolvedDraft = stateReferenceResolver
            .resolveReferences(stateDraft)
