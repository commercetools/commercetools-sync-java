...
class UnresolvedTransitionsServiceImplTest {
    ...
    void fetch_OnSuccess_ShouldReturnMock() {
        ...
        final Set<WaitingToBeResolvedTransitions> toBeResolvedOptional = service
            .fetch(singleton("state-draft-key"))
            .toCompletableFuture()
            .join();
    ...
    void delete_WithUnsuccessfulMockCtpResponse_ShouldReturnProperException() {
        ...
        final Optional<WaitingToBeResolvedTransitions> toBeResolvedOptional = service
            .delete("state-draft-key").toCompletableFuture().join();
    ...
    void delete_OnSuccess_ShouldMakeRequestWithSha1HashedKey() {
        ...
        verify(stateSyncOptions.getCtpClient()).execute(requestArgumentCaptor.capture());
        assertThat(toBeResolvedOptional).contains(waitingDraft);
        final CustomObjectDeleteCommand<WaitingToBeResolvedTransitions> value = requestArgumentCaptor.getValue();
        assertThat(value.httpRequestIntent().getPath())
            .contains(sha1Hex(stateDraftMock.getKey()));
    }
