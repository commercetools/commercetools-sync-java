...
class BaseServiceImplTest {
    ...
    private Consumer warningCallback = mock(Consumer.class);
    private SphereClient client = mock(SphereClient.class);
    private TestServiceImpl service;

    class TestServiceImpl extends BaseServiceImpl<TestDraft, TestResource, TestSyncOptions, TestQuery, TestQueryModel,
    ...
    void fetchCachedResourceId_WithFetchReturnResource_ShouldReturnResourceId() {
        ...
        Optional<String> result = service.fetchCachedResourceId(key).toCompletableFuture().join();
    ...
    void fetchMatchingResources_WithEmptyKeySet_ShouldFetchNothing() {
        ...
        assertAll(
            () -> assertThat(resources).as("Nothing should be fetched").isEmpty(),
            () -> assertThat(service.keyToIdCache).as("Nothing should be cached").isEmpty()
        );
    ...
    void fetchMatchingResources_WithKeySet_ShouldFetchResources() {
        ...
        assertAll(
            () -> assertThat(resources).as("Should return resources").isNotEmpty(),
            () -> assertThat(resources).as("Should return prepared resources").contains(mock1, mock2),
    ...
    void fetchResource_WithKey_ShouldFetchResource() {
        String resourceId = randomString();
        String resourceKey = randomString();
        ...
        assertAll(
            () -> assertThat(resourceOptional).as("Should return resource").isNotEmpty(),
            () -> assertThat(resourceOptional).as("Should return prepared resource").containsSame(mock),
            () -> assertThat(service.keyToIdCache.get(resourceKey)).as("Resource's id should be cached")
    ...
    void buildResourceKeysQueryPredicate_WithKeySet_ShouldBuildQueryPredicate() {
        ...
        assertThat(queryPredicate.toSphereQuery())
            .as("Should contain prepared keys")
    ...
    void buildResourceKeysQueryPredicate_WithKeySetContainingInvalidKeys_ShouldBuildQueryPredicate() {
        ...
        assertThat(queryPredicate.toSphereQuery())
            .as("Should contain only valid keys")
            .isIn("key in (\"" + key1 + "\", \"" + key2 + "\")", "key in (\"" + key2 + "\", \"" + key1 + "\")");
    }
