...
class InventoryBatchValidatorTest {
    ...
    void setup() {
        ...
        syncOptions = InventorySyncOptionsBuilder
            .of(ctpClient)
    ...
    @Test
    void validateAndCollectReferencedKeys_WithNullInventoryDraft_ShouldHaveValidationErrorAndEmptyResult() {
    ...
    @Test
    void validateAndCollectReferencedKeys_WithInventoryDraftWithNullKey_ShouldHaveValidationErrorAndEmptyResult() {
        final InventoryEntryDraft inventoryDraft = mock(InventoryEntryDraft.class);
        final Set<InventoryEntryDraft> validDrafts = getValidDrafts(Collections.singletonList(inventoryDraft));
    ...
    @Test
    void validateAndCollectReferencedKeys_WithInventoryDraftWithEmptySku_ShouldHaveValidationErrorAndEmptyResult() {
        final InventoryEntryDraft inventoryDraft = mock(InventoryEntryDraft.class);
    ...
    void validateAndCollectReferencedKeys_WithValidDrafts_ShouldReturnCorrectResults() {
        ...
        when(validInventoryDraft.getSupplyChannel()).thenReturn(ResourceIdentifier.ofKey("validSupplyChannelKey"));
        ...
        assertThat(errorCallBackMessages.get(0))
            .isEqualTo(INVENTORY_DRAFT_SKU_NOT_SET);
        assertThat(pair.getLeft())
            .containsExactlyInAnyOrder(validInventoryDraft, validMainInventoryDraft);
        assertThat(pair.getRight().getChannelKeys())
            .containsExactlyInAnyOrder("validSupplyChannelKey");
        assertThat(pair.getRight().getTypeKeys())
            .containsExactlyInAnyOrder("typeKey");
    }

    @Nonnull
    private Set<InventoryEntryDraft> getValidDrafts(@Nonnull final List<InventoryEntryDraft> inventoryDrafts) {
        final InventoryBatchValidator inventoryBatchValidator =
            new InventoryBatchValidator(syncOptions, syncStatistics);
        final ImmutablePair<Set<InventoryEntryDraft>, InventoryBatchValidator.ReferencedKeys> pair =
