...
public class AbstractSetCustomTypeAssert<
        T extends AbstractSetCustomTypeAssert<T, S>, S extends SetCustomTypeBase>
  ...
  public AbstractSetCustomTypeAssert hasValues(
    ...
    return ofNullable(type)
        .map(
            customType ->
                hasTypeValues(customType.getId(), customType.getKey(), customType.getTypeId()))
        .orElseGet(
            () -> {
  ...
  private AbstractSetCustomTypeAssert hasTypeValues(
      @Nullable final String id, @Nullable final String key, @Nullable final String typeId) {
