...
class GraphQlQueryAllTest {
  ...
  private static final ResourceKeyIdGraphQlResult pagedGraphQlQueryResult =
  ...
  static void setup() {
    when(sphereClient.execute(any()))
        .thenReturn(CompletableFuture.completedFuture(pagedGraphQlQueryResult));
  }
  ...
  void run_WithConsumer_ShouldApplyConsumer() {
    ...
    final GraphQlQueryAll<ResourceKeyIdGraphQlResult, ResourceKeyId> query =
        GraphQlQueryAll.of(
            sphereClient, mock(ResourceKeyIdGraphQlRequest.class), DEFAULT_PAGE_SIZE);
    final List<String> resourceIds = new ArrayList<>();

    final Consumer<Set<ResourceKeyId>> resourceIdCollector =
        page -> page.forEach(resource -> resourceIds.add(resource.getId()));
