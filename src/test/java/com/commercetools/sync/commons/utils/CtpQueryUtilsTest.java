...
class CtpQueryUtilsTest {
  ...
  @Captor
  private ArgumentCaptor<GraphQlBaseRequest<ResourceKeyIdGraphQlResult>>
      graphQlRequestArgumentCaptor;
  ...
  void queryAll_WithGraphQlRequest_ShouldFetchPagedResult() {
    // preparation
    final SphereClient sphereClient = mock(SphereClient.class);
    final List<ResourceKeyId> resultPage =
    ...
    final ResourceKeyIdGraphQlResult pagedResourceKeyIdGraphQlResult =
    ...
    final Set<String> keysToQuery =
        IntStream.range(1, 2010).mapToObj(i -> "key" + i).collect(Collectors.toSet());
