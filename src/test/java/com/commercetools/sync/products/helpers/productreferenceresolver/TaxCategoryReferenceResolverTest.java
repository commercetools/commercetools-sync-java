...
class TaxCategoryReferenceResolverTest {
  ...
  void resolveTaxCategoryReference_WithKeys_ShouldResolveReference() {
    final ProductDraftBuilder productBuilder =
        getBuilderWithRandomProductType().taxCategory(ResourceIdentifier.ofKey("taxCategoryKey"));

    final ProductDraftBuilder resolvedDraft =
        referenceResolver.resolveTaxCategoryReference(productBuilder).toCompletableFuture().join();
  ...
  void resolveTaxCategoryReference_WithNonExistentTaxCategory_ShouldNotResolveReference() {
    ...
    final String expectedMessageWithCause =
        format(
            ...
            TaxCategory.resourceTypeId(),
            "dummyKey",
            format(TAX_CATEGORY_DOES_NOT_EXIST, "nonExistentKey"));

    referenceResolver
        .resolveTaxCategoryReference(productBuilder)
        .exceptionally(
            exception -> {
