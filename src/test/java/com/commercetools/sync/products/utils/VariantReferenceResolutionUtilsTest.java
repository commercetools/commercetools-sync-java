...
class VariantReferenceResolutionUtilsTest {
  ...
  void
    ...
    final Price price =
        ProductSyncMockUtils.getPriceMockWithReferences(
    ...
    final ProductVariant productVariant =
        ProductSyncMockUtils.getProductVariantMock(singletonList(price), singletonList(asset));

    final List<ProductVariantDraft> variantDrafts =
        VariantReferenceResolutionUtils.mapToProductVariantDrafts(
            singletonList(productVariant), referenceIdToKeyCache);
  ...
  void mapToProductVariantDrafts_WithReferenceIdToKeyValuesNoneCached_ShouldNotReplaceIds() {
    ...
    final Price price =
        ProductSyncMockUtils.getPriceMockWithReferences(
            channelReference, customTypeReference, null);
    final ProductVariant productVariant =
        ProductSyncMockUtils.getProductVariantMock(singletonList(price), singletonList(asset2));
  ...
  void mapToPriceDraft_WithReferenceIdToKeyValuesNoneCached_ShouldNotReplaceIds() {
    ...
    final Price price =
        ProductSyncMockUtils.getPriceMockWithReferences(channelReference, typeReference, null);
    final ProductVariant productVariant =
        ProductSyncMockUtils.getProductVariantMock(singletonList(price));

    final List<PriceDraft> priceDrafts =
        VariantReferenceResolutionUtils.mapToPriceDrafts(productVariant, referenceIdToKeyCache);
  ...
  void mapToPriceDraft_WithReferenceIdToKeyValuesCached_ShouldReplaceIds() {
    ...
    final Price price1 =
        ProductSyncMockUtils.getPriceMockWithReferences(
            channelReference1, customTypeReference, null);
    final Price price2 =
        ProductSyncMockUtils.getPriceMockWithReferences(
            channelReference2, customTypeReference, null);

    final ProductVariant productVariant =
        ProductSyncMockUtils.getProductVariantMock(asList(price1, price2));
  ...
  void mapToVariantDraft_WithNoAttributes_ShouldNotReplaceIds() {
    ...
    final List<ProductVariantDraft> replacedDrafts =
        VariantReferenceResolutionUtils.mapToProductVariantDrafts(
            singletonList(variant), referenceIdToKeyCache);
  ...
  void mapToVariantDraft_WithAttributesWithNoReferences_ShouldNotChangeAttributes() {
    final ProductProjection product =
        ProductSyncMockUtils.createProductFromJson(
            ProductSyncMockUtils.PRODUCT_KEY_1_RESOURCE_PATH);
