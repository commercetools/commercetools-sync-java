...
class TaxCategoryUpdateActionUtilsTest {
    ...
    void buildTaxRatesUpdateActions_WithMoreSubRates_ShouldReturnOnlyReplaceAction() {
        ...
        final List<UpdateAction<TaxCategory>> result = buildTaxRateUpdateActions(taxCategory, taxCategoryDraft);
    ...
    void buildTaxRatesUpdateActions_WithDuplicatedTaxRateDrafts_ShouldReturnOnlyFirstDraftInAction() {
        ...
        TaxRateDraft duplicatedCountryCodeAndState = TaxRateDraftBuilder
            .of("12% US", 0.12, false, CountryCode.US)
            .state("state")
            .build();

        TaxCategoryDraft taxCategoryDraft = TaxCategoryDraftBuilder
            .of("name", asList(taxRateDraft, duplicatedCountryCodeAndState), "desc")
            .key("tax-category-key")
            .build();
    ...
    @Test
    void buildTaxRatesUpdateActions_WithNewTaxRateDraftsWithSameCountryAndDifferentStates_ShouldCreateCorrectActions() {
        TaxCategory taxCategory = mock(TaxCategory.class);
        when(taxCategory.getKey()).thenReturn("tax-category-key");
        when(taxCategory.getName()).thenReturn("name");
        when(taxCategory.getDescription()).thenReturn("desc");
        ...
        TaxRateDraft taxRateDraft2 = TaxRateDraftBuilder
            .of("12% US", 0.12, false, CountryCode.US)
            .state("state-2")
            .build();

        TaxRateDraft newTaxRateDraft = TaxRateDraftBuilder
            .of("13% US", 0.13, false, CountryCode.US)
            .state("state-3")
            .subRates(singletonList(subRate3))
            .build();

        TaxCategoryDraft taxCategoryDraft = TaxCategoryDraftBuilder
            .of("name", asList(taxRateDraft1, taxRateDraft2, newTaxRateDraft), "desc")
            .key("tax-category-key")
            .build();

        final List<UpdateAction<TaxCategory>> result = buildTaxRateUpdateActions(taxCategory, taxCategoryDraft);

        assertThat(result).isEqualTo(asList(RemoveTaxRate.of("taxRate-4"), AddTaxRate.of(newTaxRateDraft)));
    }
