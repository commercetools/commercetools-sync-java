package deprecated.ctpprojectsource.types;
...
import static deprecated.commons.utils.TypeITUtils.populateSourceProject;
...
class TypeSyncIT {
  ...
  /**
   * Deletes types from source and target CTP projects. Populates source and target CTP projects
   * with test data.
   */
  @BeforeEach
  void setup() {
  ...
  void sync_WithoutUpdates_ShouldReturnProperStatistics() {
    ...
    final List<Type> types =
        CTP_SOURCE_CLIENT.execute(TypeQuery.of()).toCompletableFuture().join().getResults();

    final List<TypeDraft> typeDrafts =
        types.stream()
            .map(
                type ->
                    TypeDraftBuilder.of(type.getKey(), type.getName(), type.getResourceTypeIds())
        ...
        types.stream()
            .map(
                type ->
                    TypeDraftBuilder.of(
                            type.getKey(),
                            LocalizedString.ofEnglish("updated_name"),
