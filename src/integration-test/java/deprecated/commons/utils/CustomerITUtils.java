package deprecated.commons.utils;

import static deprecated.commons.utils.CustomerGroupITUtils.createCustomerGroup;
import static deprecated.commons.utils.CustomerGroupITUtils.deleteCustomerGroups;
import static deprecated.commons.utils.ITUtils.createTypeIfNotAlreadyExisting;
import static deprecated.commons.utils.ITUtils.deleteTypes;
import static deprecated.commons.utils.ITUtils.queryAndExecute;
import static deprecated.commons.utils.StoreITUtils.createStore;
import static deprecated.commons.utils.StoreITUtils.deleteStores;
...
public final class CustomerITUtils {
  ...
  /**
   * Deletes all customers, types, stores and customer groups from the CTP project defined by the
   * {@code ctpClient}.
   *
  ...
  public static ImmutablePair<Customer, CustomerDraft> createSampleCustomerJohnDoe(
    ...
    final Type customTypeGoldMember =
        createCustomerCustomType("customer-type-gold", Locale.ENGLISH, "gold customers", ctpClient);

    final CustomerGroup customerGroupGoldMembers =
        createCustomerGroup(ctpClient, "gold members", "gold");

    final CustomerDraft customerDraftJohnDoe =
        CustomerDraftBuilder.of("john@example.com", "12345")
  ...
  public static Customer createSampleCustomerJaneDoe(@Nonnull final SphereClient ctpClient) {
    final CustomerDraft customerDraftJaneDoe =
        CustomerDraftBuilder.of("jane@example.com", "12345")
  ...
  public static Customer createCustomer(
      @Nonnull final SphereClient ctpClient, @Nonnull final CustomerDraft customerDraft) {

    final CustomerSignInResult customerSignInResult =
        executeBlocking(ctpClient.execute(CustomerCreateCommand.of(customerDraft)));
  ...
  public static Type createCustomerCustomType(
    ...
    return createTypeIfNotAlreadyExisting(
        typeKey, locale, name, ResourceTypeIdsSetBuilder.of().addCustomers(), ctpClient);
  }
