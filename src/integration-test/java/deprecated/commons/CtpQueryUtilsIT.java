package deprecated.commons;

import static com.commercetools.sync.commons.utils.CtpQueryUtils.queryAll;
import static deprecated.commons.utils.CategoryITUtils.OLD_CATEGORY_CUSTOM_TYPE_KEY;
import static deprecated.commons.utils.CategoryITUtils.createCategories;
import static deprecated.commons.utils.CategoryITUtils.createCategoriesCustomType;
import static deprecated.commons.utils.CategoryITUtils.deleteAllCategories;
import static deprecated.commons.utils.CategoryITUtils.getCategoryDraftsWithPrefix;
...
class CtpQueryUtilsIT {
  ...
  void queryAll_WithKeyCollectorConsumerOn100CategoriesWithCustomPageSize_ShouldCollectKeys() {
    ...
    final Consumer<List<Category>> categoryPageConsumer =
        categoryPageResults ->
            categoryPageResults.forEach(category -> categoryKeys.add(category.getKey()));

    queryAll(CTP_TARGET_CLIENT, CategoryQuery.of(), categoryPageConsumer, 10)
        .toCompletableFuture()
        .join();
  ...
  void queryAll_WithKeyCollectorConsumerOn600Categories_ShouldCollectKeys() {
    ...
    queryAll(CTP_TARGET_CLIENT, CategoryQuery.of(), categoryPageConsumer)
        .toCompletableFuture()
        .join();
    assertThat(categoryKeys).hasSize(numberOfCategories);
  }
  ...
  @Test
  void
      queryAll_WithCategoryCollectorCallbackWithNonUniformPageSplitting_ShouldFetchAllCategories() {
    ...
    final Set<String> pageCategoryKeys = new HashSet<>();
    categoryPages.forEach(
        page -> page.forEach(category -> pageCategoryKeys.add(category.getKey())));
    assertThat(pageCategoryKeys).hasSize(numberOfCategories);
  }
  ...
  void queryAll_WithCategoryCollectorCallbackOn600Categories_ShouldFetchAllCategories() {
    ...
    final List<List<Category>> categoryPages =
        queryAll(CTP_TARGET_CLIENT, CategoryQuery.of(), (categories -> categories))
            .toCompletableFuture()
            .join();
  ...
  void queryAll_WithIdCollectorConsumerWithSinglePage_ShouldCollectIds() {
    ...
    Set<String> allCategoryKeys =
        categories.stream().map(category -> category.getKey()).collect(Collectors.toSet());
    final List<String> categoryIds = new ArrayList<>();

    final Consumer<Set<ResourceKeyId>> categoryPageConsumer =
        categoryPageResults ->
    ...
    queryAll(CTP_TARGET_CLIENT, resourceKeyIdGraphQlRequest, categoryPageConsumer)
        .toCompletableFuture()
        .join();
  ...
  void queryAll_WithIdCollectorConsumerWithRequestedKeySubset_ShouldCollectIds() {
    final List<CategoryDraft> categoryDrafts =
        getCategoryDraftsWithPrefix(Locale.ENGLISH, "new", null, 5);
    List<Category> categories = createCategories(CTP_TARGET_CLIENT, categoryDrafts);
    ...
    ResourceKeyIdGraphQlRequest resourceKeyIdGraphQlRequest =
        new ResourceKeyIdGraphQlRequest(
            singleton(categories.get(0).getKey()), GraphQlQueryResources.CATEGORIES);
