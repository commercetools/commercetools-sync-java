...
class UnresolvedTransitionsServiceImplIT {
  ...
  void saveFetchAndDelete_WithoutExceptions_shouldWorkCorrectly() {
    ...
    final StateDraft stateDraft =
        StateDraftBuilder.of("key", StateType.LINE_ITEM_STATE).transitions(newTransitions).build();
    ...
    // test
    final Set<WaitingToBeResolvedTransitions> waitingDrafts =
        unresolvedTransitionsService
    ...
    // test
    final Optional<WaitingToBeResolvedTransitions> deletionResult =
        unresolvedTransitionsService.delete(stateDraft.getKey()).toCompletableFuture().join();
  ...
  void saveFetchAndDelete_WithKeyWithSpecialCharacter_shouldWorkCorrectly() {
    ...
    // test
    final CustomObjectByKeyGet<WaitingToBeResolvedTransitions> customObjectByKeyGet =
        CustomObjectByKeyGet.of(
            CUSTOM_OBJECT_CONTAINER_KEY,
            sha1Hex(stateDraft.getKey()),
            WaitingToBeResolvedTransitions.class);
    final CustomObject<WaitingToBeResolvedTransitions> createdCustomObject =
  ...
  void save_ExistingStateDraftWithoutException_overwritesOldCustomObjectValue() {
    ...
    // test
    final Optional<WaitingToBeResolvedTransitions> latestResult =
        unresolvedTransitionsService
        ...
        .hasValueSatisfying(
            WaitingToBeResolvedTransitions -> {
              ...
              assertThat(WaitingToBeResolvedTransitions.getMissingTransitionStateKeys())
                  .isEqualTo(
                      newStateDraftWithUnresolvedTransitions.getMissingTransitionStateKeys());
            });
