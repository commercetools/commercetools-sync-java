...
class ShoppingListServiceImplIT {
    ...
    /**
     * Deletes shopping list and products from the target CTP projects, then it populates the project with test data.
     */
    @BeforeEach
    void setup() {
    ...
    void fetchShoppingList_WithExistingShoppingList_ShouldFetchShoppingList() {
        final Optional<ShoppingList> shoppingList =
            shoppingListService.fetchShoppingList(this.shoppingList.getKey()).toCompletableFuture().join();
    ...
    void fetchMatchingShoppingListsByKeys_WithNotExistingKeys_ShouldReturnEmptySet() {
        ...
        Set<ShoppingList> shoppingLists =
            shoppingListService.fetchMatchingShoppingListsByKeys(shoppingListKeys).toCompletableFuture().join();
    ...
    void cacheKeysToIds_WithEmptyKeys_ShouldReturnCurrentCache() {
        Map<String, String> cache = shoppingListService.cacheKeysToIds(emptySet()).toCompletableFuture().join();
    ...
    void cacheKeysToIds_WithCachedKeys_ShouldReturnCachedKeysWithoutRequest() {
        ...
        cache = shoppingListService.cacheKeysToIds(singleton(shoppingList.getKey()))
                                   .toCompletableFuture().join();
        assertThat(cache).hasSize(1);
    ...
    void createShoppingList_WithValidShoppingList_ShouldCreateShoppingList() {
        ...
        final ProductDraft productDraft = ProductDraftBuilder
            .of(ResourceIdentifier.ofKey(productType.getKey()), LocalizedString.ofEnglish("newProduct"),
        ...
        final Optional<ShoppingList> createdShoppingList =
            spyShoppingListService.createShoppingList(newShoppingListDraft).toCompletableFuture().join();

        final Optional<ShoppingList> queriedOptional = CTP_TARGET_CLIENT
            .execute(ShoppingListQuery.of().withPredicates(shoppingListQueryModel ->
                shoppingListQueryModel.key().is("new_key")))
            .toCompletableFuture().join().head();

        assertThat(queriedOptional).hasValueSatisfying(queried ->
            assertThat(createdShoppingList).hasValueSatisfying(created -> {
    ...
    void createShoppingList_WithNotExistingSkuInLineItem_ShouldNotCreateShoppingList() {
        ...
        final Optional<ShoppingList> createdShoppingListOptional = shoppingListService
        ...
        assertThat(errorCallBackMessages)
            .hasSize(1);
        assertThat(errorCallBackMessages.get(0)).contains("Failed to create draft with key: 'new_key'. Reason: "
    ...
    void updateCustomer_WithValidChanges_ShouldUpdateCustomerCorrectly() {
        ...
        final ShoppingList updatedShoppingList =
            shoppingListService.updateShoppingList(shoppingList, singletonList(updatedName))
                               .toCompletableFuture().join();
        assertThat(updatedShoppingList).isNotNull();

        final Optional<ShoppingList> queried = CTP_TARGET_CLIENT
            .execute(ShoppingListQuery.of().withPredicates(shoppingListQueryModel ->
