...
class StateServiceImplIT {
    ...
    void fetchMatchingStatesByKeys_WithEmptySetOfKeys_ShouldReturnEmptySet() {
        ...
        final Set<State> matchingStates = stateService.fetchMatchingStatesByKeys(stateKeys)
            .toCompletableFuture()
            .join();
    ...
    void fetchMatchingStatesByKeys_WithAnyExistingKeys_ShouldReturnASetOfStates() {
        ...
        final State transitionState = createState(CTP_TARGET_CLIENT, "state_transition_key_1",
            TRANSITION_STATE_TYPE, null);
        final State fetchState = createState(CTP_TARGET_CLIENT, key, TRANSITION_STATE_TYPE, transitionState);
    ...
    void fetchMatchingStatesByKeys_WithAllExistingSetOfKeys_ShouldCacheFetchedStateIds() {
        final Set<State> fetchedStates = stateService.fetchMatchingStatesByKeys(singleton(OLD_STATE_KEY))
            .toCompletableFuture().join();
        assertThat(fetchedStates).hasSize(1);

        final Optional<State> stateOptional = CTP_TARGET_CLIENT
            .execute(StateQuery.of().withPredicates(queryModel -> queryModel.key().is(OLD_STATE_KEY)))
            .toCompletableFuture()
            .join()
            .head();
        ...
        stateService.updateState(stateOptional.get(), Collections.singletonList(ChangeKey.of(newKey)))
            .toCompletableFuture()
            .join();

        // Fetch cached id by old key
        final Optional<String> cachedStateId = stateService.fetchCachedStateId(OLD_STATE_KEY)
            .toCompletableFuture()
            .join();
    ...
    void fetchMatchingStatesByKeysWithTransitions_WithAnyExistingKeys_ShouldReturnASetOfStates() {
        ...
        final State transitionState = createState(CTP_TARGET_CLIENT, "state_transition_key_2",
            TRANSITION_STATE_TYPE, null);
        final State fetchState = createState(CTP_TARGET_CLIENT, key, TRANSITION_STATE_TYPE, transitionState);
        ...
        final Set<State> matchingStates = stateService.fetchMatchingStatesByKeysWithTransitions(stateKeys)
            .toCompletableFuture()
            .join();
    ...
    void createState_WithValidState_ShouldCreateStateAndCacheId() {
        final StateDraft newStateDraft = StateDraftBuilder.of(
            STATE_KEY_1,
            STATE_TYPE)
            .name(STATE_NAME_1)
        ...
        //test
        final Optional<State> createdState = spyStateService
            .createState(newStateDraft)
            .toCompletableFuture().join();

        final Optional<State> queriedOptional = CTP_TARGET_CLIENT
            .execute(StateQuery.of().withPredicates(stateQueryModel ->
                stateQueryModel.key().is(STATE_KEY_1)))
            .toCompletableFuture().join().head();

        assertThat(queriedOptional).hasValueSatisfying(queried ->
            assertThat(createdState).hasValueSatisfying(created -> {
    ...
    void updateState_WithValidChanges_ShouldUpdateStateCorrectly() {
        ...
        final State updatedState = stateService.updateState(stateOptional.get(), singletonList(setNameUpdateAction))
            .toCompletableFuture().join();
        assertThat(updatedState).isNotNull();

        final Optional<State> updatedStateOptional = CTP_TARGET_CLIENT
            .execute(StateQuery.of()
                .withPredicates(stateQueryModel -> stateQueryModel.key().is(OLD_STATE_KEY)))
            .toCompletableFuture().join().head();
    ...
    void updateState_WithInvalidChanges_ShouldCompleteExceptionally() {
        ...
        stateService.updateState(stateOptional.get(), singletonList(changeTypeUpdateAction))
    ...
    void fetchState_WithNullKey_ShouldNotFetchState() {
        final Optional<State> fetchedStateOptional =
            executeBlocking(stateService.fetchState(null));
        assertThat(fetchedStateOptional).isEmpty();
    }
