package com.commercetools.sync.integration.sdk2.externalsource.types;
...
class TypeSyncIT {
  ...
  void setup() {
    ...
    ensureTypeByTypeDraft(TYPE_DRAFT_1, CTP_TARGET_CLIENT);
  }
  ...
  void sync_WithConcurrentModificationException_ShouldRetryToUpdateNewTypeWithSuccess() {
    ...
    ensureTypeByTypeDraft(typeDraft, CTP_TARGET_CLIENT);
  ...
  void sync_WithConcurrentModificationExceptionAndFailedFetch_ShouldFailToReFetchAndUpdate() {
    ...
    CTP_TARGET_CLIENT.types().post(typeDraft).execute().join();
  ...
  void sync_WithSetOfEnumsAndSetOfLenumsChanges_ShouldUpdateTypeAddingFieldDefinition() {
    ...
    CTP_TARGET_CLIENT.types().post(oldTypeDraft).execute().join();
