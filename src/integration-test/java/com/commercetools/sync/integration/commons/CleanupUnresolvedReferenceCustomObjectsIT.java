...
class CleanupUnresolvedReferenceCustomObjectsIT {
    ...
    void cleanup_withDeleteDaysAfterLastModification_ShouldDeleteAndReturnCleanupStatistics() {
        ...
        assertThat(statistics.getReportMessage())
            .isEqualTo("Summary: 10 custom objects were deleted in total (0 failed to delete).");
    }
    ...
    void createSampleUnresolvedReferences() {
        ...
        final Set<Reference<State>> sampleTransitions = new HashSet<>(
        ...
        for (int i = 1; i <= 5; i++) {
            productUnresolvedReferences.add(new WaitingToBeResolved(
                ProductDraftBuilder.of(sampleProductDraft).key(format("productKey%s", i)).build(),
                asSet("foo", "bar")));

            transitionUnresolvedReferences.add(new WaitingToBeResolvedTransitions(
        ...
        CompletableFuture.allOf(
            CompletableFuture.allOf(
                productUnresolvedReferences
                    .stream()
                    .map(unresolvedReferencesService::save)
                    .map(CompletionStage::toCompletableFuture)
                    .toArray(CompletableFuture[]::new)),
            CompletableFuture.allOf(
                transitionUnresolvedReferences
                    .stream()
